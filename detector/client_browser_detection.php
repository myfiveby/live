...
/*
Script Name: PHP Browser/OS detection and manipulate CSS to fix height width as per client environment
Author: Shahadat Hossain Khan
Version 1.0.0
Copyright (C) 1 May 2010

Special thanks to Harald Hope (http://techpatterns.com/downloads/php_browser_detection.php)
*/

/* 
private function browser_detection( $which_test, $test_excludes='' ) 
// gets which os from the browser string
private function get_os_data ( $pv_browser_string, $pv_browser_name, $pv_version_number  )

function Info:
private function returns browser number, gecko rv number, or gecko release date
private function get_item_version( $browser_user_agent, $search_string, $substring_length )
$pv_extra_search='' allows us to set an additional search/exit loop parameter, but we
only want this running when needed 
private function get_item_version( $pv_browser_user_agent, $pv_search_string, $pv_b_break_last='', $pv_extra_search='' )

private function get_set_count( $pv_type, $pv_value='' )

// Special ID notes: Novarra-Vision is a Content Transformation Server (CTS)
private function check_is_mobile( $pv_browser_user_agent )

// thanks to this page: http://www.zytrax.com/tech/web/mobile_ids.html for data used here
private function get_mobile_data( $pv_browser_user_agent )
...
if(!defined('CURRENT_PAGE')){
	$my_cur_pg_domain=((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']=='on')?'https':'http').'://'.$_SERVER['SERVER_NAME'];
	define('CURRENT_PAGE', $my_cur_pg_domain.$_SERVER['REQUEST_URI']);
}

/* browser detection default values */
if(!defined('BDDV_SCREEN_WIDTH')) define('BDDV_SCREEN_WIDTH', 1024);
if(!defined('BDDV_SCREEN_HEIGHT')) define('BDDV_SCREEN_HEIGHT', 768);
if(!defined('BDDV_AVAIL_WIDTH')) define('BDDV_AVAIL_WIDTH', 1024);
if(!defined('BDDV_AVAIL_HEIGHT')) define('BDDV_AVAIL_HEIGHT', 738);
/*
if(!defined('BDDV_')) define('BDDV_', );
*/


class clientBrowserDetection{


	private $session_name;
	private $session_values;
	public $browser_info_php;
	private $arr_bot_agent;
	private $browser_info;
	public $browser_detection_js_meta;


	public function __construct($p_force_check=false, $p_session_name='rbd'){
		$this->session_name=$p_session_name;
		if(isset($_SESSION[$this->session_name])) $this->session_values=$_SESSION[$this->session_name];
		$this->browser_info_php=$this->browser_detection('full_assoc');
		$this->arr_bot_agent=array('bot', 'dow', 'lib');
/*		if(in_array($this->browser_info_php['ua_type'], $this->arr_bot_agent))
		echo('<pre>'.print_r($this->browser_info_php,true).'</pre>'); exit();
		if(!in_array($this->browser_info_php['ua_type'], $this->arr_bot_agent)) $this->grabClientBrowserInfo($p_force_check);*/
		if(!in_array($this->browser_info_php['ua_type'], $this->arr_bot_agent) && true==$p_force_check) $this->grabClientBrowserInfo($p_force_check);
	}

	private function getDefaultBrowserInformation(){
		$default_brow_info['CURRENT_PAGE']=CURRENT_PAGE;
		$default_brow_info['ua_type']='bro';
		$default_brow_info['isJavaScriptEnabled']=false;
		$default_brow_info['isCookieEnabled']=false;
		$default_brow_info['clientLocalData']=array('screen_width'=>BDDV_SCREEN_WIDTH, 'screen_height'=>BDDV_SCREEN_HEIGHT, 'screen_availWidth'=>BDDV_AVAIL_WIDTH, 'screen_availHeight'=>BDDV_AVAIL_HEIGHT);
		$default_brow_info['screenWidth']=BDDV_SCREEN_WIDTH;
		$default_brow_info['screenHeight']=BDDV_SCREEN_HEIGHT;
		$default_brow_info['availWidth']=BDDV_AVAIL_WIDTH;
		$default_brow_info['availHeight']=BDDV_AVAIL_HEIGHT;
/*
		$default_brow_info['']=;
*/
		return $default_brow_info;
	}

	public function getBrowserInformation(){
		$this->browser_info=$this->getDefaultBrowserInformation();
		$rtrn_browser_info=$this->browser_info;
		if(isset($this->browser_info_php) && is_array($this->browser_info_php) && count($this->browser_info_php)>0){
			$rtrn_browser_info['ua_type']=$this->browser_info_php['ua_type'];
			$rtrn_browser_info['os']=$this->browser_info_php['os'];
			$rtrn_browser_info['browser_name']=$this->browser_info_php['browser_name'];
			$rtrn_browser_info['dom']=$this->browser_info_php['dom'];
			if($this->browser_info_php['mobile_test'] && 'mobile'==$this->browser_info_php['ua_type']){
				$rtrn_browser_info['mobile']=$this->browser_info_php['mobile_test'];
				$rtrn_browser_info['mobile_vendor']=$this->browser_info_php['mobile_data'][0];
				if(strlen(trim($this->browser_info_php['mobile_data'][1]))>0) $rtrn_browser_info['mobile_browser']=$this->browser_info_php['mobile_data'][1];
				if(strlen(trim($this->browser_info_php['mobile_data'][3]))>0) $rtrn_browser_info['mobile_os']=$this->browser_info_php['mobile_data'][3];
				if(strlen(trim($this->browser_info_php['mobile_data'][7]))>0) $rtrn_browser_info['mobile_device_number']=$this->browser_info_php['mobile_data'][7];
				$rtrn_browser_info['mobile_data']=$this->browser_info_php['mobile_data'];
			}
		}
		if(isset($this->session_values) && is_array($this->session_values)){
			if (isset($this->session_values['isJavaScriptEnabled']) && $this->session_values['isJavaScriptEnabled']==true) {
				$rtrn_browser_info['isJavaScriptEnabled'] = true;
			} else if (isset($this->browser_info['isJavaScriptEnabled'])) {
				$rtrn_browser_info['isJavaScriptEnabled'] = $this->browser_info['isJavaScriptEnabled'];
			} else {
				$rtrn_browser_info['isJavaScriptEnabled'] = false;
			}
//			$rtrn_browser_info['isJavaScriptEnabled']=('true'==$this->session_values['isJavaScriptEnabled']?true:$this->browser_info['isJavaScriptEnabled']);
			if(isset($this->session_values['js_values']) && is_array($this->session_values['js_values']) && count($this->session_values['js_values'])>0){
				$rtrn_browser_info['isCookieEnabled']=('enabled'==$this->session_values['js_values']['cookie_enable_status']?true:$this->browser_info['isCookieEnabled']);
				$rtrn_browser_info['clientLocalData']=$this->session_values['js_values'];
				$rtrn_browser_info['screenWidth']=(isset($this->session_values['js_values']['screen_width'])?floatval($this->session_values['js_values']['screen_width']):$this->browser_info['screenWidth']);
				$rtrn_browser_info['screenHeight']=(isset($this->session_values['js_values']['screen_height'])?floatval($this->session_values['js_values']['screen_height']):$this->browser_info['screenHeight']);
				$rtrn_browser_info['availWidth']=(isset($this->session_values['js_values']['screen_availWidth'])?floatval($this->session_values['js_values']['screen_availWidth']):$this->browser_info['availWidth']);
				$rtrn_browser_info['availHeight']=(isset($this->session_values['js_values']['screen_availHeight'])?floatval($this->session_values['js_values']['screen_availHeight']):$this->browser_info['availHeight']);

				if(isset($this->session_values['js_values']['location_langitude']) && strlen(trim($this->session_values['js_values']['location_langitude']))>0) $rtrn_browser_info['location_langitude']=trim($this->session_values['js_values']['location_langitude']);
				if(isset($this->session_values['js_values']['location_langitude']) && strlen(trim($this->session_values['js_values']['location_langitude']))>0) $rtrn_browser_info['location_langitude']=trim($this->session_values['js_values']['location_langitude']);
			}
		}
		return $rtrn_browser_info;
	}

	public static function getQuerySap($p_pg){
		$f_qry_sap=(defined('QUERY_START')?QUERY_START:'?');
		if(preg_match("/[".$f_qry_sap."]/", $p_pg)>0) $f_qry_sap=(defined('QUERY_SAPERATOR')?QUERY_SAPERATOR:'&');
		return $f_qry_sap;
	}


	public function setSessionName($p_session_name){
		unset($_SESSION[$this->session_name]);
		$this->session_name=$p_session_name;
		$_SESSION[$this->session_name]=$this->session_values;
	}

	private function grabClientBrowserInfo($p_force_check=false){
		#echo '<h1>got it!</h1>';
		$detect_browser=!$this->isBrowserDetected();
		if(isset($_POST) && is_array($_POST) && count($_POST)<=0) $detect_browser=$p_force_check;
		if(true==$p_force_check){
			$detect_browser=true;
			$isBrowserDetectInitilized=('true'==$this->getDataFromSession('isBrowserDetectInitilized')?true:false);
			#$isDataGrabCompleteX=('true'==$this->getDataFromSession('isDataGrabComplete')?true:false);
			if(false==$isBrowserDetectInitilized) unset($_SESSION[$this->session_name]);
		}
		if(true==$detect_browser){
			#echo '<h4>browser redirect detection on process</h4>';
			$js_check_qry_var=$this->session_name.'_js_enable';
			$pg_salt_qry_var_name=$this->session_name.'_pg_salt';
			$isSetCurRefreshPage=('true'==$this->getDataFromSession('isSetCurRefreshPage')?true:false);
			if(false==$isSetCurRefreshPage){
				$base_page=$cur_refresh_page=(defined('CURRENT_PAGE')?CURRENT_PAGE:$_SERVER['PHP_SELF']);
				$this->setDataIntoSession('CURRENT_PAGE', $cur_refresh_page);
				$this->setDataIntoSession('isSetCurRefreshPage', 'true');
				$this->setDataIntoSession('isBrowserDetectInitilized', 'true');
			}else $base_page=$this->getDataFromSession('CURRENT_PAGE');
			/* step 01: check if browser enabled JS */
			$isJavaScriptCheckComplete=('true'==$this->getDataFromSession('isJavaScriptCheckComplete')?true:false);
			#var_dump($isJavaScriptCheckComplete);
			if(false==$isJavaScriptCheckComplete){
				#var_dump($_GET[$js_check_qry_var]);
				if(!isset($_GET[$js_check_qry_var])){
					/* deploy refresh code for detect JS enabled */
					$pg=$base_page.self::getQuerySap($base_page).$js_check_qry_var.'=';
					$this->browser_detection_js_meta='
<script language="javascript1.2" type="text/javascript">
window.top.location="'.$pg.'enabled";
</script>
<noscript><META http-equiv="refresh" content="0;URL='.$pg.'disabled"></noscript>
					';
					return;
//					exit();
				}else{
					$this->setDataIntoSession('java_script_enable_status', $_GET[$js_check_qry_var]);
					$js_enabled=('enabled'==$_GET[$js_check_qry_var]?'true':'false');
					$this->setDataIntoSession('isJavaScriptEnabled', $js_enabled);
					$this->setDataIntoSession('isJavaScriptCheckComplete', 'true');
				}
			}
			/* step 02: get value from JS if javascript enabled! */
			$isJavaScriptEnabled=('true'==$this->getDataFromSession('isJavaScriptEnabled')?true:false);
			$isDataGrabComplete=('true'==$this->getDataFromSession('isDataGrabComplete')?true:false);
			if(true==$isJavaScriptEnabled && false==$isDataGrabComplete){
				$data_check_qry_var=$this->session_name.'_data_grabbed';
				if(!isset($_GET[$data_check_qry_var])){
					/* deploy refresh code for detect client data */
					$pg=$base_page.self::getQuerySap($base_page);
					$this->browser_detection_js_meta='
...
function locationHandler(location){
    ...
    if(lat && lng){
		...
		client_langitude = lng;
		client_latitude = lat;
		// alert(lat+\' : \'+lng);
    }
}

var screen_width = (screen.width) ? screen.width : "";
var screen_height = (screen.height) ? screen.height : "";
// check for windows off standard dpi screen res
if (typeof(screen.deviceXDPI) == \'number\') {
	screen_width *= screen.deviceXDPI/screen.logicalXDPI;
	screen_height *= screen.deviceYDPI/screen.logicalYDPI;
} 
var screen_colorDepth = (screen.colorDepth) ? screen.colorDepth : "";
var screen_pixelDepth = (screen.pixelDepth) ? screen.pixelDepth : "";
var screen_availWidth = (screen.availWidth) ? screen.availWidth : "";
var screen_availHeight = (screen.availHeight) ? screen.availHeight : "";
var cookie_enable_status = \'disabled\';
createCookie( \''.$this->session_name.'_cookie_test\', \'it_worked\' , 1 );
if ( readCookie( \''.$this->session_name.'_cookie_test\' ) ) cookie_enable_status = \'enabled\';
...
var '.$this->session_name.'_output = "'.$this->session_name.'_width="+screen_width+"&'.$this->session_name.'_height="+screen_height+"&'.$this->session_name.'_colorDepth="+screen_colorDepth+"&'.$this->session_name.'_pixelDepth="+screen_pixelDepth+"&'.$this->session_name.'_availWidth="+screen_availWidth+"&'.$this->session_name.'_availHeight="+screen_availHeight+"&'.$this->session_name.'_cookie_enable_status="+cookie_enable_status;
if(client_latitude && client_langitude) '.$this->session_name.'_output += "&'.$this->session_name.'_lng="+client_langitude+"&'.$this->session_name.'_lat="+client_latitude;


//alert("'.$pg.'");
window.top.location="'.$pg.'"+'.$this->session_name.'_output+"&'.$data_check_qry_var.'=yes";

// End -->
</script>
<noscript><META http-equiv="refresh" content="0;URL='.$pg.$js_check_qry_var.'=disabled&'.$data_check_qry_var.'=yes"></noscript>
					';
					return;
				}else{
					$data_grabbed_through_js=('yes'==$_GET[$data_check_qry_var]?'true':'false');
					$this->setDataIntoSession('isDataGrabComplete', $data_grabbed_through_js);
					if(isset($_GET[$js_check_qry_var])){
						$this->setDataIntoSession('java_script_enable_status', $_GET[$js_check_qry_var]);
						$js_enabled=('enabled'==$_GET[$js_check_qry_var]?'true':'false');
						$this->setDataIntoSession('isJavaScriptEnabled', $js_enabled);
						$isJavaScriptEnabled=('true'==$js_enabled?true:false);
						$this->setDataIntoSession('isJavaScriptCheckComplete', 'true');
					}
					if(true==$isJavaScriptEnabled){
						$js_values['screen_width']=$_GET[$this->session_name.'_width'];
						$js_values['screen_height']=$_GET[$this->session_name.'_height'];
						$js_values['screen_colorDepth']=$_GET[$this->session_name.'_colorDepth'];
						$js_values['screen_pixelDepth']=$_GET[$this->session_name.'_pixelDepth'];
						$js_values['screen_availWidth']=$_GET[$this->session_name.'_availWidth'];
						$js_values['screen_availHeight']=$_GET[$this->session_name.'_availHeight'];
						$js_values['cookie_enable_status']=$_GET[$this->session_name.'_cookie_enable_status'];
						$js_values['location_langitude']=$_GET[$this->session_name.'_lng'];
						$js_values['location_latitude']=$_GET[$this->session_name.'_lat'];
/*
						$js_values['']=$_GET[$this->session_name.'_'];
*/
						$this->setDataIntoSession('js_values', $js_values);
					}
					if('true'==$data_grabbed_through_js){
						#$this->setDataIntoSession('isBrowserDetectInitilized', 'false');
						header('location: '.$this->getDataFromSession('CURRENT_PAGE')); exit();
					}
				}
			}
...
/*
			unset($_SESSION[$this->session_name]);
			echo('<pre>'.print_r($this->session_values,true).'<br />'.print_r($this->browser_info_php,true).'</pre>'); exit();
*/
			$this->setDataIntoSession('isBrowserDetectInitilized', 'false');
			$this->setBrowserDetected(true);
		}
	}

	/*
	 * create date: 20100930, last update date: 20120412
	 * functionality:
	 * generate javascript to load an external css with passing some browser information by java like
	 * screen width, height, color depth etc. so that any one can pass dynamic css file to adjust
	 * design for various screen resolution and color depth.
	 */
	function getCssLoadingJsCode($p_css_url, $p_meta_refresh_enable_fl=true){

		$f_noscript_query=$this->session_name.'_js_enable';
		$v_qry_start=(defined('QUERY_START')?QUERY_START:'?');
		$f_qry_sap=$v_qry_start;
		if(preg_match("/[".$v_qry_start."]/", $p_css_url)>0) $f_qry_sap=(defined('QUERY_SAPERATOR')?QUERY_SAPERATOR:'&');
...
var ".$this->session_name."_output = '".$p_css_url.$f_qry_sap.$f_noscript_query."=enabled&".$this->session_name."_width='+screen_width+'&".$this->session_name."_height='+screen_height+'&".$this->session_name."_colorDepth='+screen_colorDepth+'&".$this->session_name."_pixelDepth='+screen_pixelDepth+'&".$this->session_name."_availWidth='+screen_availWidth+'&".$this->session_name."_availHeight='+screen_availHeight+'&".$this->session_name."_cookie_enable_status='+cookie_enable_status;
if(client_latitude && client_langitude) ".$this->session_name."_output += '&".$this->session_name."_lng='+client_langitude+'&".$this->session_name."_lat='+client_latitude;

if(document.createStyleSheet) {
	document.createStyleSheet(".$this->session_name."_output);
} else {
	var newSS=document.createElement('link');
	newSS.setAttributeNode(createHtmlAttribute('rel', 'stylesheet'));
	newSS.setAttributeNode(createHtmlAttribute('type', 'text/css'));
	newSS.setAttributeNode(createHtmlAttribute('media', 'screen'));
	newSS.setAttributeNode(createHtmlAttribute('href', ".$this->session_name."_output));
...
<link rel='STYLESHEET' type='text/css' media='all' href='".$p_css_url.$f_qry_sap.$f_noscript_query."=disabled' />
";
		if(true==$p_meta_refresh_enable_fl){
			$my_cur_pg_domain=((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']=='on')?'https':'http').'://'.$_SERVER['SERVER_NAME'];
			$cur_uri=$my_cur_pg_domain.$_SERVER['REQUEST_URI'];
			$f_qry_sap=$v_qry_start;
			if(preg_match("/[".$v_qry_start."]/", $cur_uri)>0) $f_qry_sap=(defined('QUERY_SAPERATOR')?QUERY_SAPERATOR:'&');
			if((isset($_REQUEST[$f_noscript_query]) && 'disabled'!=strtolower(trim($_REQUEST[$f_noscript_query]))) || !isset($_REQUEST[$f_noscript_query])) $f_noscript=$f_noscript."<META http-equiv='refresh' content='0;URL=".$cur_uri.$f_qry_sap.$f_noscript_query."=disabled' />";
		}
		$f_noscript=$f_noscript."

</noscript>
";

		$f_output=(((isset($_REQUEST[$f_noscript_query]) && 'disabled'!=strtolower(trim($_REQUEST[$f_noscript_query]))) || !isset($_REQUEST[$f_noscript_query]))?$f_javascript:NULL).$f_noscript;
	...
	private function setBrowserDetected($p_browser_detected_fl){
		$bd_txt=(true==$p_browser_detected_fl?'true':'false');
		$this->setDataIntoSession('browser_detected', $bd_txt);
	}
	public function isBrowserDetected(){
		return ('true'==$this->getDataFromSession('browser_detected')?true:false);
	}

	private function setDataIntoSession($p_data_index, $p_data_value){
		$this->session_values[$p_data_index]=$p_data_value;
		$_SESSION[$this->session_name][$p_data_index]=$p_data_value;
	}
	private function getDataFromSession($p_data_index){
		if (isset($p_data_index) && $p_data_index!="") {
			if (isset($_SESSION[$this->session_name]) && ($_SESSION[$this->session_name] != NULL)) {
				if (isset($_SESSION[$this->session_name][$p_data_index]) && ($_SESSION[$this->session_name][$p_data_index]!=NULL)) {
					return $_SESSION[$this->session_name][$p_data_index];
				}
			}
		}
	}
...
*******************************************/
	...
	private function browser_detection( $which_test, $test_excludes='' ) 
	{
		...
		*/
		if ( !$b_repeat )
		{
			...
			*/
			$a_moz_types = array( 'bonecho', 'camino', 'epiphany', 'firebird', 'flock', 'galeon', 'iceape', 'icecat', 'k-meleon', 'minimo', 'multizilla', 'phoenix', 'songbird', 'swiftfox', 'seamonkey', 'shiretoko', 'iceweasel', 'firefox', 'netscape6', 'netscape', 'rv', 'waterfox' );
					...
					switch ( $browser_working )
					{
							...
							*/
							...
							for ( $j = 0; $j < $j_count; $j++ )
							{
								if ( strstr( $browser_user_agent, $a_moz_types[$j] ) )
								{
									...
									$moz_number = $this->get_item_version( $browser_user_agent, $moz_type );
									break;
								}
							}
							/*
						...
						case 'ie':
							...
							*/
							...
							if ( strstr( $browser_number, '7.' ) && strstr( $browser_user_agent, 'trident/4' ) )
							{
	...
	*/
	private function check_is_mobile( $pv_browser_user_agent )
	{
		...
		*/
		// os
		'android', 'epoc', 'linux armv', 'palmos', 'palmsource', 'windows ce', 'symbianos', 'symbian os', 'symbian', 'webos', 
		// devices ipod before iphone or fails
		'benq', 'blackberry', 'danger hiptop', 'ddipocket', ' droid', 'htc_dream', 'htc hero', 'ipod', 'iphone', 'kindle', 'lge-cx', 'lge-lx', 'lge-mx', 'lge vx', 'lge ', 'lge-', 'lg;lx', 'nintendo wii', 'nokia', 'palm', 'pdxgw', 'playstation', 'sagem', 'samsung', 'sec-sgh', 'sharp', 'sonyericsson', 'sprint', 'j-phone', 'n410', 'mot 24', 'mot-', 'htc-', 'htc_', 'sec-', 'sie-m', 'sie-s', 'spv ', 'vodaphone', 'smartphone', 'armv', 'midp', 'mobilephone',
		// browsers
