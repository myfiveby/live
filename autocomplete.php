...
$q = $_GET['q'];
$comprueba_exists_frases = Requete ("SELECT title,id_myf FROM myfive_panels   WHERE   title LIKE  $q " , $connexion);
 
 if (mysqli_num_rows($comprueba_exists_frases) ==0 ){
 
 echo" ";} else {
 
 $datos_frases = mysqli_fetch_array($comprueba_exists_frases);
 
 echo $datos_frases['title'];
 }
